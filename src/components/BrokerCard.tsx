  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {data.map((b) => (
          <div key={b.broker} className="space-y-1.5">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium capitalize">{b.broker}</span>
              <span className={cn('text-sm font-semibold', b.pnl >= 0 ? 'text-green-400' : 'text-red-400')}>
            ...
            <div className="h-1.5 rounded-full bg-accent overflow-hidden">
              <div
                className={cn('h-full rounded-full transition-all', b.pnl >= 0 ? 'bg-green-500' : 'bg-red-500')}
